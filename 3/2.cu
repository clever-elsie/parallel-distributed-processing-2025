...
__global__ void max_idx(int*array, int*max_idx){
  ...
  for(int i=0;i<loop;++i){
    ...
    if(idx<N)
      local_max=max(local_max, array[idx]);
  }
  atomicMax(s_max+(threadIdx.x&31), local_max);
  __syncthreads();
  if(threadIdx.x<32)
    #if 1
    for(int i=1;i<32;i<<=1)
...
int which(int argc, char** argv){
  if(argc!=2) return 0;
  if(std::string_view("one_path")==argv[1]) return 0;
  else if(std::string_view("two_path")==argv[1]) return 1;
